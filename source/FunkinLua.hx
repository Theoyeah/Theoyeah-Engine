...
class FunkinLua {
	...
	public function couldntFindObject(vars:Dynamic, from:String) {
		#if LUA_ALLOWED
		var varr:String = vars.toString;
		luaTrace('$from: Couldnt find object: $varr', false, false, FlxColor.RED);
		#else
		lol();
		#end
	}
...
//_____________ more options things ____________________\\
		...
		Lua_helper.add_callback(lua, "openCustomSubstate", function(name:String, pauseGame:Bool = false) {
			if(name.length < 1 || name == null) {
				luaTrace("openCustomSubstate: there isn't any name for param " + '"name"', false, false, FlxColor.RED);
				return; // fuck it
			}
			if(pauseGame)
			{
		...
		Lua_helper.add_callback(lua, "setSpriteShader", function(obj:String, shader:String) {
			...
			{
				luaTrace('setSpriteShader: Shader $shader is missing! Initialize it with initLuaShader', false, false, FlxColor.RED);
				return false;
			}
		...
		Lua_helper.add_callback(lua, "setGlobalFromScript", function(luaFile:String, global:String, val:Dynamic) { // sets the global from a script
			var cervix = luaFile + ".lua";
			if(luaFile.endsWith(".lua")) cervix = luaFile;
			var doPush = false;
			#if MODS_ALLOWED
		...
		Lua_helper.add_callback(lua, "addLuaScript", function(luaFile:String, ignoreAlreadyRunning:Bool = false) { //would be dope asf. 
			...
			luaTrace('addLuaScript: Script "$luaFile" ' + "doesn't exist!");
		});
		...
		Lua_helper.add_callback(lua, "removeLuaScript", function(luaFile:String, ignoreAlreadyRunning:Bool = false) { //would be dope asf. 
			...
			if(doPush)
			{
				if(!ignoreAlreadyRunning)
				{
					for (luaInstance in PlayState.instance.luaArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							//luaTrace('removeLuaScript: The script "' + cervix + '" is already running!');
			...
			luaTrace('removeLuaScript: Script "$luaFile" doesn' + "'t exist!");
		});
		...
		Lua_helper.add_callback(lua, "runHaxeCode", function(codeToRun:String) {
			...
			try {
				...
				luaTrace('runHaxeCode: ' + scriptName + ":" + lastCalledFunction + " - " + e, false, false, FlxColor.RED);
			}
		...
		Lua_helper.add_callback(lua, "addHaxeLibrary", function(libName:String, ?libPackage:String = '') {
			...
			try {
				...
				luaTrace('addHaxeLibrary: ' + scriptName + ":" + lastCalledFunction + " - " + e);
			}
			#end
		});

		Lua_helper.add_callback(lua, "loadSong", function(?name:String, difficultyNum:Int = -1, newLoadSystem:Bool = true) {
			if(name == null || name.length < 1)
				name = PlayState.SONG.song;
			if (difficultyNum == -1)
				difficultyNum = PlayState.storyDifficulty;

			var poop = Highscore.formatSong(name, difficultyNum, newLoadSystem);
			PlayState.SONG = Song.loadFromJson(poop, name, newLoadSystem);
		...
		Lua_helper.add_callback(lua, "loadGraphic", function(variable:String, image:String, ?gridX:Int, ?gridY:Int, where:String = 'images') {
 			...
 			{
 				spr.loadGraphic(Paths.image(image, null, where));
 				spr.loadGraphic(Paths.image(image, null, where), animated, gridX, gridY);
 			}
 		});
		Lua_helper.add_callback(lua, "loadFrames", function(variable:String, image:String, spriteType:String = "sparrow", where:String = 'images') {
			var killMe:Array<String> = variable.split('.');
			var spr:FlxSprite = getObjectDirectly(killMe[0]);
			if(killMe.length > 1) {
				spr = getVarInArray(getPropertyLoopThingWhatever(killMe), killMe[killMe.length-1]);
			...
			if(spr != null && image != null && image.length > 0)
			{
				loadFrames(spr, image, spriteType, where);
			}
		});
		...
		Lua_helper.add_callback(lua, "getPropertyFromGroup", function(obj:String, index:Int, variable:Dynamic) {
			...
			luaTrace("getPropertyFromGroup: Object #" + index + " from group: " + obj + " doesn't exist!");
			Lua.pushnil(lua);
			return null;
		});
		...
		Lua_helper.add_callback(lua, "getObjectOrder", function(obj:String) {
			...
			luaTrace('getObjectOrder: Object $obj doesnt exist!');
			return -1;
		});
		Lua_helper.add_callback(lua, "setObjectOrder", function(obj:String, position:Int) {
			...
			luaTrace('setObjectOrder: Object $obj doesnt exist!');
		});

		// gay ass tweens
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			...
			if(penisExam != null) {
				PlayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {x: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						if(callLuas) PlayState.instance.callOnLuas('onTweenCompleted', [tag]);
						PlayState.instance.modchartTweens.remove(tag);
					}
				}));
			} else
				couldntFindObject(vars, 'doTweenX');
		});
		Lua_helper.add_callback(lua, "doTweenY", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				...
				couldntFindObject(vars, 'doTweenY');
		});
		Lua_helper.add_callback(lua, "doTweenAngle", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				...
				couldntFindObject(vars, 'doTweenAngle');
		});
		Lua_helper.add_callback(lua, "doTweenAlpha", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				...
				couldntFindObject(vars, 'doTweenAlpha');
		});
		Lua_helper.add_callback(lua, "doTweenZoom", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				...
				couldntFindObject(vars, 'doTweenZoom');
		});
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String, callLuas:Bool = true) {
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				...
				couldntFindObject(vars, 'doTweenColor');
		});

		//Tween shit, but for strums
		Lua_helper.add_callback(lua, "noteTweenX", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			cancelTween(tag);
			if(note < 0) note = 0;
		...
		Lua_helper.add_callback(lua, "noteTweenY", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes.members[note % PlayState.instance.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "noteTweenDirection", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes.members[note % PlayState.instance.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "noteTweenAngle", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes.members[note % PlayState.instance.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "noteTweenAlpha", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String, callLuas:Bool = true) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes.members[note % PlayState.instance.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "precacheImage", function(name:String) { // use this please!! Theres a lot of lag when you dont use this before!
		...
		Lua_helper.add_callback(lua, "exitSong", function(skipTransition:Bool, songToPlay:String = 'freakyMenu', goTo:String = 'automatic') {
			if(skipTransition)
			{
			...
			switch (goTo.trim().toLowerCase().replace(' ', '')) {
				case 'true' | 'freeplay' | 'freeplaymenustate' | 'freeplaymenu':
					MusicBeatState.switchState(new FreeplayState());
				case 'false' | 'story' | 'storymenustate' | 'storymenu':
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float, antialiasing:String = 'automatic', where:String = 'images') {
			/*
			...
			if(image != null && image.length > 0)
				leSprite.loadGraphic(Paths.image(image, null, where));

			switch(antialiasing.toLowerCase().trim()) {
				case 'true': leSprite.antialiasing = true;
				case 'false': leSprite.antialiasing = false;
				default: leSprite.antialiasing = ClientPrefs.globalAntialiasing;
			}
			PlayState.instance.modchartSprites.set(tag, leSprite);
			leSprite.active = true;
		});
		Lua_helper.add_callback(lua, "makeAnimatedLuaSprite", function(tag:String, image:String, x:Float, y:Float, spriteType:String = 'sparrow', antialiasing:String = 'automatic', where:String = 'images') {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:ModchartSprite = new ModchartSprite(x, y);

			loadFrames(leSprite, image, spriteType, where);
			switch(antialiasing.toLowerCase().trim()) {
				case 'true': leSprite.antialiasing = true;
				case 'false': leSprite.antialiasing = false;
				default: leSprite.antialiasing = ClientPrefs.globalAntialiasing;
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			...
			luaTrace('setGraphicSize: Couldnt find object: ' + obj);
		});
		Lua_helper.add_callback(lua, "scaleObject", function(obj:String, x:Float, y:Float, updateHitbox:Bool = true) {
			...
			luaTrace('scaleObject: Couldnt find object: ' + obj);
		});
		...
		/*Lua_helper.add_callback(lua, "isNoteChild", function(parentID:Int, childID:Int) { // sorry :'(
			...
			luaTrace('isNoteChild: ${parentID} or ${childID} is not a valid note ID');
			return false;
		});*/
		...
		Lua_helper.add_callback(lua, "setObjectCamera", function(obj:String, camera:String = '') {
			...
			luaTrace("setObjectCamera: Object " + obj + " doesn't exist!");
			return false;
		});
		...
		Lua_helper.add_callback(lua, "screenCenter", function(obj:String, pos:String = 'xy') {
			...
			luaTrace("screenCenter: Object " + obj + " doesn't exist!");
		});
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null) {
			...
			if(FileSystem.exists(path)) {
				...
				if(shit.dialogue.length > 0) {
					...
					luaTrace('startDialogue: Successfully loaded dialogue "$dialogueFile"');
				} else
					luaTrace('startDialogue: Dialogue file "$dialogueFile" is badly formatted!');
			} else {
				luaTrace('startDialogue: Dialogue file "' + dialogueFile + '" not found');
		...
		Lua_helper.add_callback(lua, "startVideo", function(videoFile:String) {
			...
			#else
			luaTrace('startVideo: videos are not allowed');
			if(PlayState.instance.endingSong)
				PlayState.instance.endSong();
			else
				PlayState.instance.startCountdown();
		...
		Lua_helper.add_callback(lua, "playSound", function(sound:String, volume:Float = 1, ?tag:String = null, callLuas:Bool = true) {
			if(tag != null && tag.length > 0) {
				...
				PlayState.instance.modchartSounds.set(tag, FlxG.sound.play(Paths.sound(sound), volume, false, function() {
					...
					if(callLuas) PlayState.instance.callOnLuas('onSoundFinished', [tag]);
				}));
				return;
			}
		...
		Lua_helper.add_callback(lua, "setSoundTime", function(tag:String, value:Float) {
			var tagger:String = if(PlayState.instance.modchartSounds.exists(tag)) tag else tag.toLowerCase();
			if(tag != null && tag.length > 0 && PlayState.instance.modchartSounds.exists(tagger)) {
		...
		Lua_helper.add_callback(lua, "debugPrintArray", function(text:Array<Dynamic>) { // better for arrays
			var thisText:Dynamic = '';
			for (daText in text) {
				thisText += daText;
			}
			luaTrace(thisText, true, false);
		});
		...
		Lua_helper.add_callback(lua, "textExists", function(tag) {
			return (getTextObject(tag) != null);
		});
		...
		Lua_helper.add_callback(lua, "setTextAutoSize", function(tag:String, autoSize:Bool) {
			var obj:FlxText = getTextObject(tag);
			if(obj != null) {
		...
		Lua_helper.add_callback(lua, "setTextBorderQuality", function(tag:String, borderQuality:Float) {
			var obj:FlxText = getTextObject(tag);
			if(obj != null) {
				obj.borderQuality = borderQuality;
				return true;
			}
			luaTrace("setTextBorderQuality: Object " + tag + " doesn't exist!", false, false, FlxColor.RED);
			return false;
		});
		...
		Lua_helper.add_callback(lua, "setTextBold", function(tag:String, bold:Bool) {
			var obj:FlxText = getTextObject(tag);
			if(obj != null) {
				obj.bold = bold;
				return true;
			}
			luaTrace("setTextBold:: Object " + tag + " doesn't exist!", false, false, FlxColor.RED);
			return false;
		});
		...
		Lua_helper.add_callback(lua, "getTextAlignment", function(tag:String) {
			var obj:FlxText = getTextObject(tag);
			var align:String = '';
			if(obj != null) {
			...
			luaTrace("getTextAlignment: Object " + tag + " doesn't exist!", false, false, FlxColor.RED);
			return align;
		});
		Lua_helper.add_callback(lua, "getTextItalic", function(tag:String, defaultReturn:Dynamic = false) {
			var obj:FlxText = getTextObject(tag);
			if(obj != null)
				return obj.italic;
			luaTrace("getTextItalic: Object " + tag + " doesn't exist!", false, false, FlxColor.RED);
			return defaultReturn;
		});

		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			var tagger:String = if(PlayState.instance.modchartTexts.exists(tag)) tag else tag.toLowerCase();
		...
		Lua_helper.add_callback(lua, "flushSaveData", function(name:String) {
			...
			luaTrace('flushSaveData: Save file not initialized: $name');
		});
		Lua_helper.add_callback(lua, "getDataFromSave", function(name:String, field:String, ?defaultValue:Dynamic = null) {
			...
			luaTrace('getDataFromSave: Save file not initialized: $name');
			return defaultValue;
		});
		Lua_helper.add_callback(lua, "setDataFromSave", function(name:String, field:String, value:Dynamic) {
			...
			luaTrace('setDataFromSave: Save file not initialized: $name');
		});
		...
		Lua_helper.add_callback(lua, "saveFile", function(path:String, content:String, absolute:Bool = false)
		{
			try {
				...
				luaTrace("saveFile: Error trying to save " + path + ": " + e);
			}
			return false;
		});
		Lua_helper.add_callback(lua, "deleteFile", function(path:String, ignoreModFolders:Bool = false, tryLowerCase:Bool = false)
		{
			try {
				...
				luaTrace("deleteFile: Error trying to delete " + path + ": " + e);
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "objectPlayAnimation", function(obj:String, name:String, forced:Bool = false, ?startFrame:Int = 0) {
			deprecated("objectPlayAnimation", "playAnim");
			if(PlayState.instance.getLuaObject(obj, false) != null) {
		...
		Lua_helper.add_callback(lua, "characterPlayAnim", function(character:String, anim:String, ?forced:Bool = false) {
			deprecated("characterPlayAnim", "playAnim");
			switch(charactersStuff(character)) {
				case 'dad':
		...
		Lua_helper.add_callback(lua, "luaSpritePlayAnimation", function(tag:String, name:String, forced:Bool = false) {
			deprecated('luaSpritePlayAnimation', 'playAnim');
			if(PlayState.instance.modchartSprites.exists(tag)) {
		...
		Lua_helper.add_callback(lua, "stringLength", function(str:String) {
			return str.length;
		});
		Lua_helper.add_callback(lua, "stringLtrim", function(str:String) {
			return str.ltrim();
		});
		Lua_helper.add_callback(lua, "stringRtrim", function(str:String) {
			return str.rtrim();
		});
		Lua_helper.add_callback(lua, "stringTrim", function(str:String) {
			return str.trim();
		});
		Lua_helper.add_callback(lua, "stringReplace", function(str:String, sub:String = ' ', by:String = '') {
			return str.replace(sub, by);
		});
		Lua_helper.add_callback(lua, "stringIsSpace", function(str:String, pos:Int = 0) {
			return str.isSpace(pos);
		});
		Lua_helper.add_callback(lua, "stringHtmlEscape", function(str:String, ?quotes:Bool) {
			return str.htmlEscape(quotes);
		});
		Lua_helper.add_callback(lua, "stringHex", function(number:Int, ?digits:Int) {
		...
		// im gonna regret doing this
		Lua_helper.add_callback(lua, "trace", function(t:Dynamic) {
			trace(t);
		});
	...
	function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		{
			luaTrace('initLuaShader: Shader $name was already initialized!');
			return true;
		}
	...
	function loadFrames(spr:FlxSprite, image:String, spriteType:String, where:String = 'images')
	{
		switch(spriteType.toLowerCase().trim())
		{
			...
			case "packer" | "packeratlas" | "pac":
				spr.frames = Paths.getPackerAtlas(image, null, where);

			default:
				spr.frames = Paths.getSparrowAtlas(image, null, where);
		}
	}
	...
	public function set(variable:String, data:Dynamic) {
		...
		Lua.setglobal(lua, variable.toUpperCase());
		#else
		lol();
		#end
	}
