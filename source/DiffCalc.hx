...
class DiffCalc
{
	...
	public static function CalculateDiff(song:SwagSong, ?accuracy:Float = .93)
	{
		...
		for (section in song.notes) // sections
		{
			for (note in section.sectionNotes) // notes
			{
				if (note[1] > 3 && !section.mustHitSection)
					cleanedNotes.push(new SmallNote(note[0], Math.floor(Math.abs(note[1]))));
				else if (note[1] < 4 && section.mustHitSection)
					cleanedNotes.push(new SmallNote(note[0], Math.floor(Math.abs(note[1]))));
			}
		}
		...
		for (clean in cleanedNotes)
		{
			clean.strumTime = ((clean.strumTime - firstNoteTime) * 2);
		}

		for (clean in cleanedNotes)
		{
			switch (clean.noteData % 4)
			{
				case 0 | 1:
					handOne.push(clean);
				case 2 | 3:
		...
		for (note in handOne)
		{
			var index = Std.int((note.strumTime * 2) / 1000);
			if (index + 1 > length)
				continue;
		...
		return Conductor.truncateFloat(chisel(accuracy, hand_diffOne, hand_diffTwo, point_npsOne, point_npsTwo, maxPoints), 2);
	}
