 ...
 */
class OldAlphabet extends FlxSpriteGroup
{
	...
	var lastSprite:OldAlphaCharacter;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	public var isBold:Bool = false;
	public var lettersArray:Array<OldAlphaCharacter> = [];
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = OldAlphaCharacter.numbers.indexOf(character) != -1;
			var isSymbol:Bool = OldAlphaCharacter.symbols.indexOf(character) != -1;
			var isAlphabet:Bool = OldAlphaCharacter.alphabet.indexOf(character.toLowerCase()) != -1;
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:OldAlphaCharacter = new AlphaCharacter(30 * loopNum, 0, textSize);
				var letter:OldAlphaCharacter = new OldAlphaCharacter(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isNumber:Bool = OldAlphaCharacter.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = OldAlphaCharacter.symbols.indexOf(splitWords[loopNum]) != -1;
			var isAlphabet:Bool = OldAlphaCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:OldAlphaCharacter = new OldAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:OldAlphaCharacter = new OldAlphaCharacter(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class OldAlphaCharacter extends FlxSprite
{
