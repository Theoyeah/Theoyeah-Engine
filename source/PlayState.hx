...
class PlayState extends MusicBeatState
{
	...
	public var windowAlerts:Bool = true;
	...
	override public function create()
	{
		...
		windowAlerts = ClientPrefs.getGameplaySetting('windowalerts', true);
		#else
		practiceMode = false;
		cpuControlled = false;
		windowAlerts = true;
		if(ClientPrefs.getGameplaySetting('healhgain', 1) > 1) healthGain = 1;
		if(ClientPrefs.getGameplaySetting('healthloss', 1) < 1) healthLoss = 1;
		#end
	...
	public function clearShaderFromCamera(cam:String) {
 	...
 	public function getLuaObject(tag:String, text:Bool = true, tryLowerCase:Bool = true, tryUpperCase:Bool = true):FlxSprite {
		...
		if(tryLowerCase) {
			...
			if(variables.exists(tag.toLowerCase())) return variables.get(tag.toLowerCase());
		}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if (ClientPrefs.instantRespawn = true) {
				...
				if(!note.noMissAnimation)
				{
					switch(note.noteType) {
						case 'Crash Note':
							...
							endSong();
							if(windowAlerts) {
								lime.app.Application.current.window.alert( 'Oops ! You pressed a crash note !!!', 'Ooops !');
							}
							System.exit(0);
	...
	public function spawnNoteSplashOnNote(note:Note, animationsCount:Int = 3) {
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievementsToCheck.length) {
			...
			if (achievementName == othersCodeName
				...
				&& (CoolUtil.difficultyString().toUpperCase() == 'HARD' || CoolUtil.difficultyString().toUpperCase() == 'FUCKED')
